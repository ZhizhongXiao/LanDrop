...
class BottleApplicationTests(unittest.TestCase):
    ...
    def test_pair_download_and_upload(self) -> None:
        ...
        self.assertIn('id="selectAllDownloads"', decoded)
        self.assertIn(">名称</span>", decoded)
        self.assertIn(">修改日期</span>", decoded)
        self.assertIn(">类型</span>", decoded)
        self.assertIn(">大小</span>", decoded)
        self.assertIn('class="file-type">TXT 文件', decoded)
        self.assertIn("selectAllDownloads.indeterminate", decoded)
